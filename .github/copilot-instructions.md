...
## Proceso de Publicación e Instalación
### Proyecto GestionTime Desktop v1.1.0 (.NET 8 / WinUI 3)
- **COMANDO DE PUBLICACIÓN:**dotnet publish GestionTime.Desktop.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -p:PublishReadyToRun=true -o "publish\portable"
- **ARCHIVOS CRÍTICOS QUE DEBEN INCLUIRSE:**
  1. GestionTime.Desktop.pri (recursos XAML compilados) - Si no se copia automáticamente, copiar desde: `bin\x64\Release\net8.0-windows10.0.19041.0\win-x64\GestionTime.Desktop.pri`
  2. Carpeta Assets/ completa (14 imágenes: login_fondoClaro.png, diario_bg_Claro.png, LogoClaro.png, etc.)
  3. window-config.ini (debe estar DENTRO de la carpeta App/ del instalador)
  4. appsettings.json

- **CONFIGURACIÓN DEL .csproj:**
  - Target "CopyPriFile" AfterTargets="Publish" para copiar el .pri
  - `<Content Include="Assets\**\*.*">` con `CopyToOutputDirectory="PreserveNewest"`

- **ESTRUCTURA DEL PAQUETE ZIP FINAL:**GestionTime-v1.1.0.zip
